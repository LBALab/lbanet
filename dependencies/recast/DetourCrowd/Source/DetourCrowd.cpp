...
int dtCrowd::addAgent(const float* pos, const dtCrowdAgentParams* params)
{
	...
	ag->state = DT_CROWDAGENT_STATE_EXTERNAL;//DT_CROWDAGENT_STATE_WALKING;
	ag->active = 1;
...
void dtCrowd::update(const float dt, dtCrowdAgentDebugInfo* debug)
{
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		if (ag->state == DT_CROWDAGENT_STATE_WALKING)
			integrate(ag, dt);
		else if (ag->state == DT_CROWDAGENT_STATE_EXTERNAL)
		{
			// Integrate manual speed
			dtVmad(ag->npos, ag->npos, ag->vel, dt);
		}
	}
...
bool dtCrowd::requestMoveVelocity(const int idx, const float* vel)
{
	...
	if(ag->state != DT_CROWDAGENT_STATE_EXTERNAL)
		return false;
...
void dtCrowd::requestSetResetTarget(const int idx, bool set)
{
	if (idx < 0 || idx > m_maxAgents)
		return;
	...
	if(set)
	{
		...
		float nearest[3];
		dtPolyRef ref;
		m_navquery->findNearestPoly(ag->npos, m_ext, &m_filter, &ref, nearest);
		if (!ref)
		{
			// Could not find a location on navmesh.
			return;
		...
		ag->state = DT_CROWDAGENT_STATE_EXTERNAL;
}